...
export default {
    ...
    mounted(){var vm = this;
        ...
        firebase.firestore().collection('Public').doc("Posts").collection('Flats').doc(this.id).get().then((doc)=>{
            if(doc.exists){
                vm.plot = doc.data();
    ...
    methods: {
        SubmitRequest(){
            ...
            firebase.firestore().collection("Interested").doc('UserPosts').collection('Flats').add({
                from: {
                    name: vm.name,
                    email: mail,
                    phone: vm.phone
