<template>
    <div class="plots">
        <h2 v-if="adminPlots.length>0">Recommended Flats</h2>
...
export default {
    ...
    methods:{
        fetchPlots(){ var vm = this;
            firebase.firestore().collection("Public").doc('Posts').collection('Flats').get().then((doc)=>{
                doc.forEach((doc)=>{
                    var v = {
                        id: doc.id,
                        plot: doc.data() 
        ...
        fetchAdminPLots(){var vm = this;
            firebase.firestore().collection("Admin").doc("Posts").collection("Flats").get().then((doc)=>{
                doc.forEach((doc)=>{
                    var v = {
                        id: doc.id,
                        plot: doc.data()
