<template>
    <div class="plots">
        <h2 v-if="adminPlots.length>0">Recommended Houses</h2>
        <div @click="plotClicked(plot.id)" v-if="adminPlots.length>0" class="adminPlots plotList">
            <div v-for="(plot, index) in adminPlots" :key="index" class="plot card">
...
export default {
    ...
    methods:{
        fetchPlots(){ var vm = this;
            firebase.firestore().collection("Public").doc('Posts').collection('Houses').get().then((doc)=>{
                doc.forEach((doc)=>{
                    var v = {
                        id: doc.id,
                        plot: doc.data() 
        ...
        fetchAdminPLots(){var vm = this;
            firebase.firestore().collection("Admin").doc("Posts").collection("Houses").get().then((doc)=>{
                doc.forEach((doc)=>{
                    var v = {
                        id: doc.id,
                        plot: doc.data()
