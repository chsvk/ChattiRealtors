<template>
    <div class="plot">
        ...
        <h1 v-if="noPlot">Sorry This URL is Invalid. Or the Flat You are searching for could have been deleted</h1>
        <div v-else class="details">
            <h1>{{plot.Name}}</h1>
            <div class="">
                <agile v-if="!(plot.DisplayImages[0]=='') && imageReady" :autoplay="true" :arrows="false" :dots="false" :speed=1000>
                    <div v-for="(image, index) in plot.DisplayImages" :key="index" class="slide">
            ...
            <div class="flex">
                <div class="data">
                    ...
                    <h4>Area: <span>{{plot.BuiltUpArea}}</span></h4>
                    <h4>Construction Status: <span>{{plot.CStatus}}</span></h4>
                    <h4>Carpet Area: <span>{{plot.Carpet}}</span></h4>
                    <h4>Bathrooms: <span>{{plot.BathRooms}}</span></h4>
                    <h4>Bedrooms: <span>{{plot.Bedrooms}}</span></h4>
                    <h4>Bus Route: <span>{{plot.BusRoute}}</span></h4>
                    <h4>Floor Number: <span>{{plot.Floor}}</span></h4>
                    <h4>Total Floors: <span>{{plot.FloorCount}}</span></h4>
                    <h4>Age of Property: <span>{{plot.PropertyAge}}</span></h4>
                </div>
                <div class="data">
                    <h4>Amenities</h4>
...
export default {
    ...
    mounted(){var vm = this;
        ...
        firebase.firestore().collection('Public').doc("Posts").collection('Houses').doc(this.id).get().then((doc)=>{
            if(doc.exists){
                vm.plot = doc.data();
    ...
    methods: {
        SubmitRequest(){
            ...
            firebase.firestore().collection("Interested").doc('UserPosts').collection('Houses').add({
                from: {
                    name: vm.name,
                    email: mail,
                    phone: vm.phone
