    ...
    <div class="plot">
        ...
        <div v-else class="details">
            <h1>{{plot.Name}}</h1>
                ...
                <h2>Gallery</h2>
                <div class="flex">
                    <Hooper v-if="!(DisplayImages[0]=='')">
                        <slide v-for="(image, index) in DisplayImages" :key="index" >
                            <img :src="image" alt="">
                        </slide>
                    </Hooper>
                    <Hooper v-if="!(plot.DevelopmentImages[1]=='')">
                        <slide v-for="(image, index) in plot.DevelopmentImages" :key="index" >
                            <img :src="image" alt="">
                        </slide>
                    </Hooper>
                    <!-- <agile v-if="!(DisplayImages[0]=='') && imageReady" :autoplay="true" :arrows="false" :dots="false" :speed=1000>
...
export default {
    ...
    mounted(){var vm = this;
        ...
        firebase.firestore().collection('Admin').doc("Posts").collection('Plots').doc(this.id).get().then((doc)=>{
            if(doc.exists){
                vm.plot = doc.data();
                vm.plot = {...vm.plot, DevelopmentImages: vm.plot.DevelopmentImages.splice(0,1)}
